...
getup.maps.event = {
  ...
  event_info_template: '<div id="polling-booths-info-box"><a href="#" data-order={{order}}><p class="name">{{name}}</p><p><strong>{{suburb}}</strong></p><p class="info">{{#distance}}<strong>Distance {{distance}}</strong>.</p> {{/distance}}' +
    '<p>Select your electorate:</p>' +
    '{{#htv_links}}' +
    '<div id="scorecard-button">' +
    '<button class="btn btn-large btn-orange scorecard-button" onclick="ga(\'send\', \'event\', \'ozvote2\', \'htv\', \'download\');" data-link="{{{link}}}" data-electorate="{{name}}"><span>{{name}}</span></button>' +
    '</div>' +
    '{{/htv_links}}' +
    '<div class="clear"></div></a></div>',

  event_results_template: '<li id="search-result-{{order}}"><span class="event-name event-center">{{name}}<span class="event-tagline event-center"><br />{{street}}, {{suburb}} {{#distance}}({{distance}}){{/distance}}</span></span>',
