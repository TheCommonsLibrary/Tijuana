...
(function($) {
    ...
    $.widget("ui.toggleEdit", {
        ...
        _init: function() {
            ...
            /*
             CHANGE - changed to also exclude reset buttons
             */
            //skip submit buttons and hidden fields
            if (self.element.is(":submit") || self.element.is(":reset") || self.element.is("input[type=hidden]")) {
                return false;
            }
            //cleanup
        ...
        edit: function(event, init) {
            ...
            self.p.hide();
            self.element.show();
            /* CHANGE - focus element a put cursor at the end of text when entering edit mode */
            self.element.focus().val(self.element.val());
            /* END OF CHANGE */
            if (!init) {
            ...
            for (i = 0;
                ...
                if ($.isFunction(self.options.types[i].is) && $.isFunction(self.options.types[i].text)) {
                    if (self.options.types[i].is(self.element)) {
                        ...
                        /*
                        CHANGE - changed logical test to include trimmed strings
                        */
                        return (val && val.trim() !== '') ? val : self.options.defaultText;
                    }
                }
            }
