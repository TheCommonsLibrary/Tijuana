...
describe "with a member that clicks the 'make my donation recurring' checkbox", type: :feature, js: true do
  before :each do
    # cache_db do
      seed
      ...
      add_page :title => "getup asks for help" do
        add_module 'Sidebar', 'Add a donation' do
          ...
          within_fieldset('Make Recurring Checkbox') do
            check 'Enabled'
          end
        end
      end
    ...
    visit campaign_path
    click_button '$30'
    check 'Make my donation monthly'
  ...
  specify 'should make one-off paypal donations monthly' do
    ...
    page.should have_content("t3: 'M'")
  end

  specify 'should make one-off credit card donations monthly' do
    ...
    donations = User.find_by_email('mel@member.com').donations.recurring.active
    expect(donations.count).to eq(1)
    expect(donations.last.frequency).to eq('monthly')
    expect(donations.last.amount_in_dollars).to eq(30)
  end
end
